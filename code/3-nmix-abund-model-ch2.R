...
## for OSS and ENES, fitted to real field data.
...
## ------------------------------------------------------------
  ...
  n_unknown <- sals %>%
    filter(spp == spp_code, recap != "1", age_class == "U") %>%
    nrow()
  cat(spp_code, "individuals with unknown age class excluded:", n_unknown, "\n")
...
## ------------------------------------------------------------
  ...
  #   1 = JW   2 = JB   3 = BZ   4 = RMM   5 = SLG   6 = VO (volunteers CH + LS combined)
...
## ------------------------------------------------------------
  ...
  NimModel <- nimbleCode({
    ...
    sigma.obs ~ dexp(1)
    for (o in 1:nobs) {
      eps.obs[o] ~ dnorm(0, sd = sigma.obs)   # det can deviate per observer level
    }
